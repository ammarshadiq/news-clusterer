 ...
 * @author shadiq
 */
public class ClusterLabelingMaxTFIDFByCluster implements ClusterLabeling {
  ...
  public ClusterLabelingMaxTFIDFByCluster(Configuration appConf, ClusterStat clustStat,
    ...
    for (String phrase : phrases) {
      double tfValue = clustStat.getClusterPhraseFrequency().get(phrase);
      double dfValue = clustStat.getClusterPhraseDocumentFrequency().get(phrase);
  ...
  public String getClusterLabel() {
    if (sortedClusterLabels.length > 0) {
      ...
      // if there is no label candidate selected due to no phrase that met the frequent phrases filter requirement
      return "EMPTY_LABEL"
              + " (" + this.clustStat.getNumOfDoc() + ")";
    }
  }
